...
if mode == "🎮 What-If Simulation":
    st.title("🎮 What-If Simulation Mode — Play with features")
    st.markdown("Use the sliders to change feature values and watch model predictions update instantly.")

    # Determine feature names for sliders (prefer scaler.feature_names_in_ if available)
    ...
    if not feature_names:
        feature_names = ["rpm_1engin", "N1__1_pcnt", "N1__2_pcnt", "OILT1__deg", "OILT2__deg", "EGT_1__deg", "EGT_2__deg", "FUEP1__psi"]

    # Sidebar control area (meter + config)
    st.sidebar.markdown("### 🧭 Simulation Controls")

    # Sidebar gauge placeholder
    sidebar_gauge_ph = st.sidebar.empty()
    ...
    if "sim_values" not in st.session_state:
        st.session_state.sim_values = {fn: sample_stats.get(fn, {"mean": 0.0})["mean"] for fn in feature_names}

    # UI sliders (main panel)
    sim_cols = st.columns(2)
    idx = 0
    for fn in feature_names:
        stats = sample_stats.get(fn, {"min": 0.0, "max": 1.0, "mean": 0.0})
        lo, hi = stats["min"], stats["max"]
        if hi <= lo: hi = lo + 0.01
        default = st.session_state.sim_values.get(fn, stats["mean"])
        step = (hi - lo) / 200 if hi - lo > 0 else 0.01
        with sim_cols[idx % 2]:
            val = st.slider(fn, lo, hi, default, step=step, key=f"sim_{fn}")
    ...
    sim_df = pd.DataFrame([{k: st.session_state.sim_values[k] for k in feature_names}])
    ...
    try:
        ...
        lstm_prob = float(lstm_model.predict(np.array(st.session_state.sim_seq_buf[-DEFAULT_LSTM_TIMESTEPS:]).reshape(1, DEFAULT_LSTM_TIMESTEPS, -1), verbose=0)[0][0]) if (len(st.session_state.sim_seq_buf) >= DEFAULT_LSTM_TIMESTEPS and lstm_model) else 0.0
    except Exception:
        lstm_prob = 0.0

    combined_sim = (xgb_prob + lstm_prob)

    # Sidebar Gauge (🎯 moved here!)
    with st.sidebar:
        st.markdown("### 🎯 Failure Probability")
        render_gauge(combined_sim, sim_g_threshold, sim_y_threshold)

    # Metrics below sliders
